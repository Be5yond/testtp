...
## 默认参数
- - -
### Session级默认参数，Session继承自requests.Session.利用requests.Session实现默认数据。
``` 
...
- - - 
### 变量缓存 --- cache
“{{}}"字符串括起来的数据被认为是变量，运行时将替换成cache中的对应数据。   
...
```
### 上下文数据缓存 --- stash
将上次请求返回的数据抽取出来，存储到cache，供后面请求使用
```
s = Session()
...
s.post('http://httpbin.org/post', json=body)
>>> OUTPUT
2020-12-06 22:07:41.644 | SEND     | testtp.Req:_wrapper:35 - <= request body => 
{
    "context_vars": "httpbin.org"
}
```
### 使用函数实时计算参数 --- register_render
“{{}}"字符串括起来的数据被认为是变量，运行时将执行函数来生成数据。   
默认支持python内置函数例如random.randint()， 也可以用register_render将自定义函数注册给渲染器使用。 
```
def get_timestamp():
        return int(time.time())
s = Session()
s.register_render(get_timestamp) 
body={'data': 'testdata', 'timestamp': '{% get_timestamp() %}'}
s.post('http://httpbin.org/post', json=body)
>>> OUTPUT
2020-12-06 22:36:32.784 | SEND     | testtp.Req:_wrapper:35 - <= request body => 
{
    "data": "testdata",
    "timestamp": 1607265392  # 实际参数为get_timestamp函数返回的当前时间戳
}
```
## 数据校验
- - -
